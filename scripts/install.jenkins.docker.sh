...
docker run \
  ...
  --volume $JENKINS_CONFIG_DIR/jenkins-docker-certs:/certs/client \
  --volume $JENKINS_CONFIG_DIR/jenkins-data:/var/jenkins_home \
  --publish 2376:2376 \
  docker:dind \
...
docker run \
  ...
  --publish 50000:50000 \
  --volume  $JENKINS_CONFIG_DIR/jenkins-data:/var/jenkins_home \
  --volume  $JENKINS_CONFIG_DIR/jenkins-docker-certs:/certs/client:ro \
  myjenkins:lts
